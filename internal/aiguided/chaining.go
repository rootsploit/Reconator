...
// AnalyzeAttackPaths uses the dedicated attack path analyzer for graph-based analysis
func (vc *VulnChainer) AnalyzeAttackPaths(vulns []Vulnerability) ([]attackpath.ScoredPath, *attackpath.AttackGraph) {
	// Convert aiguided.Vulnerability to vulnscan.Vulnerability
	var vsVulns []vulnscan.Vulnerability
	for _, v := range vulns {
		vsVulns = append(vsVulns, vulnscan.Vulnerability{
			Host:       v.Host,
			URL:        v.URL,
			TemplateID: v.TemplateID,
			Name:       v.Name,
	...
	analyzer := attackpath.NewAnalyzer()
	return analyzer.Analyze(vsVulns)
}
