...
type Result struct {
	...
	Checkpoint1         *AIDecisions      `json:"checkpoint1,omitempty"`   // From Surface Analyzer
	Checkpoint2         *AIDecisions      `json:"checkpoint2,omitempty"`   // From Scan Optimizer
...
func (s *Scanner) Scan(hosts []string, ctx *TargetContext) (*Result, error) {
	...
	// Generate attack paths from chain analysis
	if result.ChainAnalysis != nil {
		result.AttackPaths = generateAttackPaths(result.ChainAnalysis)
		if len(result.AttackPaths) > 0 {
			fmt.Printf("        [Attack Paths] Generated %d attack paths from chain analysis\n", len(result.AttackPaths))
		}
	}
...
// buildPrompt uses TOON format for efficient token usage
func (s *Scanner) buildPrompt(ctx *TargetContext) string {
	...
	// Build compact TOON data using the encoder
	data := map[string]interface{}{
		"domain":           ctx.Domain,
		"waf":              wafStatus,
		"endpoints":        len(ctx.Endpoints),
		"js_files":         len(ctx.JSFiles),
		"api":              len(ctx.APIEndpoints),
		"cdn_hosts":        ctx.CDNHosts,
		"tech":             ctx.Technologies,
		"services":         ctx.Services,
		"endpoints_sample": truncateSample(ctx.Endpoints, 8),
		"js_sample":        truncateSample(ctx.JSFiles, 4),
		"api_sample":       truncateSample(ctx.APIEndpoints, 4),
	}

	// Simple compact prompt
	return `You are a vulnerability scanner. Given this target info, recommend up to 10 Nuclei tags.

` + ToonObject(data) + `

Respond JSON: {"tags":["..."],"reason":"..."}`
}
...
// generateAttackPaths converts chain analysis results into structured attack paths.
// This is a rule-based conversion (no AI call) since the AI was already used during chain analysis.
func generateAttackPaths(analysis *ChainAnalysis) []AttackPath {
	var paths []AttackPath

	for _, chain := range analysis.Chains {
		// Map chain severity to risk level
		riskLevel := strings.ToLower(chain.Severity)
		if riskLevel == "" {
			riskLevel = "medium"
		}

		// Map likelihood string to probability
		var probability float64
		switch strings.ToLower(chain.Likelihood) {
		case "high", "very high":
			probability = 0.8
		case "medium":
			probability = 0.5
		...
		// Determine entry point from the first vulnerability with role "entry_point" or order 1
		entryPoint := ""
		for _, v := range chain.Vulns {
			if v.Role == "entry_point" || v.Order == 1 {
				entryPoint = fmt.Sprintf("%s on %s", v.Name, v.Host)
				break
			}
		}
		if entryPoint == "" && len(chain.Vulns) > 0 {
			entryPoint = fmt.Sprintf("%s on %s", chain.Vulns[0].Name, chain.Vulns[0].Host)
		}

		// Convert exploitation steps to AttackStep with order numbers
		var steps []AttackStep
		for i, stepDesc := range chain.Steps {
			step := AttackStep{
				Order:       i + 1,
				Description: stepDesc,
			}
			// Try to associate a technique and target from the chain vulns
			if i < len(chain.Vulns) {
				step.Target = chain.Vulns[i].Host
				if chain.Vulns[i].Role != "" {
					step.Technique = chain.Vulns[i].Role
				}
			}
			steps = append(steps, step)
		}

		// Collect affected assets from chain vulnerabilities
		assetSet := make(map[string]bool)
		for _, v := range chain.Vulns {
			if v.Host != "" {
				assetSet[v.Host] = true
			}
		}
		var affectedAssets []string
		for asset := range assetSet {
			affectedAssets = append(affectedAssets, asset)
		}
