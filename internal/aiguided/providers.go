...
// Priority order: Ollama -> Minimax -> Kimi -> Groq -> DeepSeek -> Claude -> OpenAI -> Gemini
func (pm *ProviderManager) LoadFromEnv() {
	// 1. Ollama (local, free, private) - FIRST PRIORITY
	...
	// 2. Minimax (cost-effective, multilingual)
	if keys := getEnvKeys("MINIMAX_API_KEY", "MINIMAX_API_KEYS"); len(keys) > 0 {
		pm.addProvider(ProviderMinimax, keys, "", "MiniMax-Text-01", 60)
	}

	// 3. Kimi / Moonshot (advanced reasoning)
	if keys := getEnvKeys("KIMI_API_KEY", "MOONSHOT_API_KEY", "KIMI_API_KEYS"); len(keys) > 0 {
		pm.addProvider(ProviderKimi, keys, "", "kimi-k2.5", 60)
	}

	// 4. Groq (ultra-fast inference)
	if keys := getEnvKeys("GROQ_API_KEY", "GROQ_API_KEYS"); len(keys) > 0 {
		pm.addProvider(ProviderGroq, keys, "", "llama-3.3-70b-specdec", 30)
	}

	// 5. DeepSeek (budget option)
	if keys := getEnvKeys("DEEPSEEK_API_KEY", "DEEPSEEK_API_KEYS"); len(keys) > 0 {
		pm.addProvider(ProviderDeepSeek, keys, "", "deepseek-chat", 60)
	}

	// 6. Claude (complex reasoning)
...
func (pm *ProviderManager) queryProviderRaw(provider ProviderType, prompt string) (string, error) {
	...
	case ProviderMinimax:
		response, err = queryMinimaxRaw(prompt, apiKey, model)
	case ProviderKimi:
		response, err = queryKimiRaw(prompt, apiKey, model)
	case ProviderOllama:
...
func (pm *ProviderManager) queryProvider(provider ProviderType, prompt string) (*AIRecommendation, error) {
	...
	case ProviderMinimax:
		rec, err = queryMinimaxWithKey(prompt, apiKey, model)
	case ProviderKimi:
		rec, err = queryKimiWithKey(prompt, apiKey, model)
	default:
		err = fmt.Errorf("unknown provider: %s", provider)
	}
...
func queryMinimaxWithKey(prompt, apiKey, model string) (*AIRecommendation, error) {
	if model == "" {
	...
	body, _ := json.Marshal(reqBody)
	req, _ := http.NewRequest("POST", "https://api.minimaxi.chat/v1/chat/completions", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+apiKey)
...
func queryKimiWithKey(prompt, apiKey, model string) (*AIRecommendation, error) {
	if model == "" {
		model = "kimi-k2.5"
	}
	...
	body, _ := json.Marshal(reqBody)
	req, _ := http.NewRequest("POST", "https://api.moonshot.cn/v1/chat/completions", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+apiKey)
	...
	if resp.StatusCode != 200 {
		respBody, _ := io.ReadAll(resp.Body)
		return nil, fmt.Errorf("Kimi error: %d - %s", resp.StatusCode, string(respBody))
	}
...
func queryMinimaxRaw(prompt, apiKey, model string) (string, error) {
	if model == "" {
		model = "MiniMax-Text-01"
	}
	...
	if resp.StatusCode != 200 {
		respBody, _ := io.ReadAll(resp.Body)
		return "", fmt.Errorf("Minimax error: %d - %s", resp.StatusCode, string(respBody))
	}
...
func queryKimiRaw(prompt, apiKey, model string) (string, error) {
	if model == "" {
		model = "kimi-k2.5"
	}
	...
	if resp.StatusCode != 200 {
		respBody, _ := io.ReadAll(resp.Body)
		return "", fmt.Errorf("Kimi error: %d - %s", resp.StatusCode, string(respBody))
	}
...
providers:
  # Ollama (local, free, private) - FIRST PRIORITY
  # Install: https://ollama.ai
  # No API key needed, just run: ollama serve && ollama pull kimi-k2
  - name: ollama
    endpoint: http://localhost:11434
    model: kimi-k2
    # model: qwen2.5:32b  # Better for complex analysis

  # Minimax - Cost-effective, multilingual
  # Get key at: https://www.minimaxi.com/
  - name: minimax
    keys:
  ...
  # Kimi / Moonshot - Advanced reasoning
