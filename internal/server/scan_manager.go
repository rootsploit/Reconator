...
func (m *ScanManager) StartScan(target string, phases []string, threads int, deepScan, passiveMode bool, opts ...StartScanOptions) (*Scan, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	// Apply distributed scanning and IP rotation options if provided
	if len(opts) > 0 {
		o := opts[0]
		cfg.DistributedScan = o.Distributed
		cfg.DistProvider = o.Provider
		if o.Workers > 0 {
			cfg.DistWorkers = o.Workers
		}
		cfg.DistSpot = o.Spot
		cfg.DistRegion = o.Region
		cfg.DistSize = o.Size
		cfg.IPRotateEnabled = o.IPRotate
		cfg.IPRotateRegions = o.IPRotateRegions
	}
