...
func (m *ScanManager) LoadReport(scan *Scan) (*report.Data, error) {
	...
	if d := loadJSON[subdomain.Result](filepath.Join(scanResultsDir, "1-subdomains", "subdomains.json")); d != nil {
		data.Subdomain = d
	}

	// Load WAF results
	if d := loadJSON[waf.Result](filepath.Join(scanResultsDir, "2-waf", "waf_detection.json")); d != nil {
		data.WAF = d
	}

	// Load port results
	if d := loadJSON[portscan.Result](filepath.Join(scanResultsDir, "3-ports", "port_scan.json")); d != nil {
		data.Ports = d
	}

	// Load takeover results
	if d := loadJSON[takeover.Result](filepath.Join(scanResultsDir, "4-takeover", "takeover.json")); d != nil {
		data.Takeover = d
	}

	// Load historic results
	if d := loadJSON[historic.Result](filepath.Join(scanResultsDir, "5-historic", "historic_urls.json")); d != nil {
		data.Historic = d
	}

	// Load tech results
	if d := loadJSON[techdetect.Result](filepath.Join(scanResultsDir, "6-tech", "tech_detection.json")); d != nil {
		data.Tech = d
	}

	// Load security headers results
	if d := loadJSON[secheaders.Result](filepath.Join(scanResultsDir, "6b-secheaders", "security_headers.json")); d != nil {
		data.SecHeaders = d
	}

	// Load dirbrute results
	if d := loadJSON[dirbrute.Result](filepath.Join(scanResultsDir, "7-dirbrute", "dirbrute.json")); d != nil {
		data.DirBrute = d
	}

	// Load JS analysis results
	if d := loadJSON[jsanalysis.Result](filepath.Join(scanResultsDir, "7b-jsanalysis", "js_analysis.json")); d != nil {
		data.JSAnalysis = d
	}

	// Load vulnscan results
	if d := loadJSON[vulnscan.Result](filepath.Join(scanResultsDir, "8-vulnscan", "vulnerabilities.json")); d != nil {
		data.VulnScan = d
	}

	// Load AI-guided results
	if d := loadJSON[aiguided.Result](filepath.Join(scanResultsDir, "9-aiguided", "ai_guided.json")); d != nil {
		data.AIGuided = d
	}

	// Load screenshot results (try both paths for compatibility)
	if d := loadJSON[screenshot.Result](filepath.Join(scanResultsDir, "9-screenshots", "screenshot_results.json")); d != nil {
		data.Screenshot = d
	} else if d := loadJSON[screenshot.Result](filepath.Join(scanResultsDir, "screenshots", "screenshot_clusters.json")); d != nil {
		// Legacy path fallback
		data.Screenshot = d
	}

	// Load IP range results
	if d := loadJSON[iprange.Result](filepath.Join(scanResultsDir, "0-iprange", "ip_discovery.json")); d != nil {
		data.IPRange = d
	}
