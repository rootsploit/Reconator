...
type StartScanRequest struct {
	...
	Provider    string `json:"provider,omitempty"` // aws, digitalocean
	Workers     int    `json:"workers,omitempty"`  // default: 3
	Spot        bool   `json:"spot,omitempty"`
	Region      string `json:"region,omitempty"`
	Size        string `json:"size,omitempty"`
	// IP rotation
	IPRotate        bool   `json:"iprotate,omitempty"`
	IPRotateRegions string `json:"iprotate_regions,omitempty"`
}

// StartScanOptions holds optional distributed scanning and IP rotation parameters
...
func (s *Server) startScan(c *gin.Context) {
	...
	// Validate mutual exclusivity: --iprotate and --distributed cannot be used together
	if req.IPRotate && req.Distributed {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "Cannot use both IP rotation (--iprotate) and distributed scanning (--distributed) at the same time. Choose one.",
		})
		return
	}

	// Create scan
	scan, err := s.scanMgr.StartScan(target, req.Phases, req.Threads, req.DeepScan, req.PassiveMode, StartScanOptions{
		...
		Region:          req.Region,
		Size:            req.Size,
		IPRotate:        req.IPRotate,
		IPRotateRegions: req.IPRotateRegions,
	})
	if err != nil {
