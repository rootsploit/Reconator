...
type PipelineRunner struct {
	...
	jsonlEmitter *output.JSONLEmitter         // JSONL streaming (nil if not enabled)
	jsonBuilder  *output.UnifiedResultBuilder // Unified JSON builder (nil if not enabled)
}
...
func (r *PipelineRunner) Run() error {
	...
	// Calculate maxConcurrent (used for parallel processing)
...
func (r *PipelineRunner) processTarget(target string) error {
	...
	// Initialize progress tracker (respects debug/silent mode internally)
	r.progress = NewPhaseProgress(r.cfg.Debug, r.cfg.Silent)
	...
	// Print target header (unless silent mode)
	if !r.cfg.Silent {
		cyan.Printf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
	...
	// Initialize JSON output modes
	if r.cfg.JSONLOutput {
		r.jsonlEmitter = output.NewJSONLEmitter(os.Stdout)
		r.jsonlEmitter.EmitScanStart(target, r.cfg.Phases)
	}
	if r.cfg.JSONOutput {
		r.jsonBuilder = output.NewUnifiedResultBuilder(target, version.Version, r.out.ScanID())
	}
	...
	for _, group := range groups {
		...
		for _, phase := range runnablePhases {
			...
			go func(p pipeline.Phase) {
				...
				// Emit JSONL phase start event
				if r.jsonlEmitter != nil {
					r.jsonlEmitter.EmitPhaseStart(string(p))
				}
				...
				// Emit JSONL phase lifecycle events
				if r.jsonlEmitter != nil {
					if result.Error != nil {
						r.jsonlEmitter.EmitError(string(p), result.Error)
					} else {
						r.jsonlEmitter.EmitPhaseComplete(string(p), map[string]interface{}{
							"duration_sec": time.Since(phaseStart).Seconds(),
						})
					}
				}

				// Track phase in unified JSON builder
				if r.jsonBuilder != nil {
					r.jsonBuilder.AddPhase(string(p))
				}
	...
	// Output unified JSON if requested
	if r.jsonBuilder != nil {
		jsonData, err := r.jsonBuilder.Build()
		if err == nil {
			fmt.Fprintln(os.Stdout, string(jsonData))
		}
	}

	// Emit scan complete JSONL event
	if r.jsonlEmitter != nil {
		r.jsonlEmitter.EmitScanComplete(target, time.Since(start), nil)
	}

	// Print summary (progress tracker handles debug mode)
	r.progress.PrintSummary(time.Since(start), r.out.BaseDir())
...
func (r *PipelineRunner) getPhasesToRun() map[pipeline.Phase]bool {
	...
	if runAll {
		...
		phaseMap := map[string]pipeline.Phase{
			...
			"aicheckpoint1": pipeline.PhaseAICheckpoint1,
			"aicheckpoint2": pipeline.PhaseAICheckpoint2,
		}
...
func (r *PipelineRunner) shouldSkipPhase(phase pipeline.Phase) bool {
	...
	case pipeline.PhaseAICheckpoint1, pipeline.PhaseAICheckpoint2:
		// AI checkpoints auto-skip internally when no keys configured
		// but also skip in passive mode
		if r.cfg.PassiveMode {
			return true
		}
	}
...
func (r *PipelineRunner) savePhaseResult(phase pipeline.Phase, result *pipeline.PhaseResult) {
	...
	case pipeline.PhaseAICheckpoint1, pipeline.PhaseAICheckpoint2:
		if data, ok := result.Data.(*aiguided.AIDecisions); ok {
			r.out.SaveAIDecisions(data)
		}
	}
}
...
func (r *PipelineRunner) generateReport(target string, results map[pipeline.Phase]*pipeline.PhaseResult, start time.Time) {
	...
	if res, ok := results[pipeline.PhaseAIGuided]; ok && res.Status == pipeline.StatusCompleted {
		if data, ok := res.Data.(*aiguided.Result); ok {
			...
			if data.AttackPaths != nil {
				reportData.AttackPaths = data.AttackPaths
			}
		}
	}
	...
	// Run OSINT inline (not a pipeline phase)
	if r.cfg.EnableOSINT {
		osc := osint.NewScanner(r.cfg, r.checker)
		if res, err := osc.Scan(target); err == nil {
			reportData.OSINTResult = res
			osc.SaveResults(res, outDir)
		}
	}

	// Run attack path analysis if we have vulnerability data
	if reportData.VulnScan != nil && len(reportData.VulnScan.Vulnerabilities) > 0 {
		apAnalyzer := attackpath.NewAnalyzer()
		scoredPaths, graph := apAnalyzer.Analyze(reportData.VulnScan.Vulnerabilities)
...
func (r *PipelineRunner) loadExistingResultsForReport(outDir string, reportData *report.Data) {
	...
	// Load OSINT results if missing
	if reportData.OSINTResult == nil {
		if data := loadJSON[osint.Result](filepath.Join(outDir, "osint", "osint_results.json")); data != nil {
			reportData.OSINTResult = data
		}
	}
}
...
func (r *PipelineRunner) extractResultCounts(phase pipeline.Phase, result *pipeline.PhaseResult) map[string]int {
	...
	case pipeline.PhasePorts:
		if data, ok := result.Data.(*portscan.Result); ok {
			...
			counts["cdn"] = len(data.CDNHosts)
			counts["direct"] = len(data.NonCDNHosts)
		}
	case pipeline.PhaseTakeover:
		if data, ok := result.Data.(*takeover.Result); ok {
	...
	case pipeline.PhaseAICheckpoint1, pipeline.PhaseAICheckpoint2:
		if data, ok := result.Data.(*aiguided.AIDecisions); ok {
			counts["hosts"] = len(data.PrioritizedHosts)
			counts["wordlist"] = len(data.GetAllWordlistEntries())
			counts["tags"] = len(data.NucleiTags)
		}
	}
