...
func (r *PipelineRunner) processTarget(target string) error {
	...
	// Initialize output directory and storage
	outDir := filepath.Join(r.cfg.OutputDir, target)
	if err := os.MkdirAll(outDir, 0755); err != nil {
		return err
	}

	// Get SQLite storage to check for interrupted scans
	...
	if r.cfg.EnableSQLite {
		var err error
		sqliteDB, err = storage.NewSQLiteStorage(outDir)
	...
	// Use resumed scan ID or create new
	if resumeScanID != "" {
		r.out.SetScanID(resumeScanID)
	}
