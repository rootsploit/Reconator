...
func (r *Runner) processWithContext(ctx context.Context, target string, progressCh chan<- ProgressUpdate, baseProgress int) error {
	...
	if r.cfg.GenerateReport {
		...
		// Run attack path analysis if we have vulnerabilities
		if vulnRes != nil && len(vulnRes.Vulnerabilities) > 0 {
			apAnalyzer := attackpath.NewAnalyzer()
			scoredPaths, graph := apAnalyzer.Analyze(vulnRes.Vulnerabilities)
			reportData.ScoredAttackPaths = scoredPaths
			if graph != nil {
				for _, n := range graph.Nodes {
					reportData.AttackGraphNodes = append(reportData.AttackGraphNodes, n)
				}
				reportData.AttackGraphEdges = graph.Edges
			}
		}
...
func (r *Runner) process(target string) error {
	...
	if r.cfg.ShouldRunPhase("subdomain") || r.cfg.ShouldRunPhase("all") {
		...
		// Print each discovered subdomain (subfinder-style)
		for _, sub := range subs {
			output.PrintSubdomain(sub, "")
		}
		fmt.Println()
	...
	if osintRes != nil {
		// Save OSINT results to disk
		osc := osint.NewScanner(r.cfg, r.c)
		osc.SaveResults(osintRes, outDir)

		// Print OSINT findings in nuclei-style format
		if osintRes.WHOIS != nil {
			detail := osintRes.WHOIS.OrgName
			if osintRes.WHOIS.Registrar != "" {
				detail += ", Registrar: " + osintRes.WHOIS.Registrar
			}
			output.PrintFinding("whois", "info", target, detail)
		}
		if osintRes.ASN != nil {
			detail := fmt.Sprintf("AS%d %s", osintRes.ASN.ASN, osintRes.ASN.OrgName)
			if osintRes.ASN.Provider != "" {
				detail += " [" + osintRes.ASN.Provider + "]"
			}
			output.PrintFinding("asn", "info", target, detail)
		}
		if len(osintRes.CertTransparency) > 0 {
			output.PrintFinding("cert", "info", target, fmt.Sprintf("%d certificates in CT logs", len(osintRes.CertTransparency)))
		}
		if len(osintRes.BreachData) > 0 {
			totalRecords := 0
			for _, b := range osintRes.BreachData {
				totalRecords += b.RecordCount
			}
			detail := fmt.Sprintf("%d known breaches", len(osintRes.BreachData))
			if totalRecords > 0 {
				detail += fmt.Sprintf(", %d records", totalRecords)
			}
			output.PrintFinding("breach", "high", target, detail)
		}
		if osintRes.DNSRecords != nil {
			dns := osintRes.DNSRecords
			if dns.SPF != "" {
				output.PrintFinding("spf", "info", target, dns.SPF)
			} else {
				output.PrintFinding("spf", "medium", target, "NO SPF RECORD")
			}
			if dns.DMARC != "" {
				output.PrintFinding("dmarc", "info", target, dns.DMARC)
			} else {
				output.PrintFinding("dmarc", "medium", target, "NO DMARC RECORD")
			}
			if len(dns.DKIM) > 0 {
				output.PrintFinding("dkim", "info", target, fmt.Sprintf("%d selector(s) found", len(dns.DKIM)))
			}
		}
	}
	fmt.Println()
	...
	if !r.cfg.PassiveMode && len(alive) > 0 {
		...
		if techRes != nil {
			// Print per-host tech detection (httpx-style)
			for host, techs := range techRes.TechByHost {
				output.PrintTech(host, techs)
			}
	...
	if !r.cfg.PassiveMode && !r.cfg.SkipVulnScan && (r.cfg.ShouldRunPhase("vulnscan") || r.cfg.ShouldRunPhase("all")) && len(alive) > 0 {
		...
		if portsRes != nil && (len(portsRes.NonCDNHosts) > 0 || len(portsRes.CDNHosts) > 0) {
			...
			if res, err := vs.ScanWithCDNPriority(alive, categorizedURLs, techInput, cdnInput); err == nil {
				vulnRes = res
				// Print each vuln in nuclei-style format
				for _, v := range res.Vulnerabilities {
					host := v.Host
					if v.URL != "" {
						host = v.URL
					}
					output.PrintFinding(v.TemplateID, v.Severity, host, v.Name)
				}
				green.Printf("\n    Vulnerabilities: %d (Critical: %d, High: %d)\n\n",
	...
	// Print compact data-rich summary box
	duration := time.Since(start).Round(time.Second)
	fmt.Println()
	cyan.Printf("╔══════════════════════════════════════════════════════════════╗\n")
	cyan.Printf("║  RECONATOR %s — %s — %s\n", version.Version, target, duration)
	cyan.Printf("╠══════════════════════════════════════════════════════════════╣\n")
	if subRes != nil {
		cyan.Printf("║  Subdomains: %d found │ %d alive\n", subRes.TotalAll, len(subs))
	}
	if portsRes != nil {
		cyan.Printf("║  Ports:      %d open │ %d alive hosts\n", portsRes.TotalPorts, len(alive))
	}
	if vulnRes != nil {
		cyan.Printf("║  Vulns:      %d critical │ %d high │ %d medium │ %d low\n",
			vulnRes.BySeverity["critical"], vulnRes.BySeverity["high"],
			vulnRes.BySeverity["medium"], vulnRes.BySeverity["low"])
	}
	if osintRes != nil {
		whoisMark := "✗"
		if osintRes.WHOIS != nil {
		...
		cyan.Printf("║  OSINT:      WHOIS %s │ ASN %s │ %d certs │ %d breaches\n",
			whoisMark, asnMark, len(osintRes.CertTransparency), len(osintRes.BreachData))
	}
	if vulnRes != nil && len(vulnRes.Vulnerabilities) > 0 {
		apAnalyzer := attackpath.NewAnalyzer()
		paths, _ := apAnalyzer.Analyze(vulnRes.Vulnerabilities)
		if len(paths) > 0 {
			critPaths, highPaths := 0, 0
			for _, p := range paths {
				if p.Impact == "critical" {
					critPaths++
				} else if p.Impact == "high" {
					highPaths++
				}
			}
			cyan.Printf("║  Paths:      %d attack chains (%d critical, %d high)\n", len(paths), critPaths, highPaths)
		}
	}
	if r.cfg.GenerateReport {
		cyan.Printf("╠══════════════════════════════════════════════════════════════╣\n")
		cyan.Printf("║  Report: report_%s.html\n", target)
	}
	cyan.Printf("╚══════════════════════════════════════════════════════════════╝\n\n")
