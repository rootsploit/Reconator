...
var falsePositiveTemplates = map[string]bool{
	...
	"options-method": true,
	"trace-method":   true,
	"tech-detect":    true, // Already handled by separate tech detection phase

	// WAF detection is informational noise
	"waf-detect":     true,
	"dns-waf-detect": true,
}
...
var nucleiParallelScanTypes = []NucleiScanType{
	...
	// Security Info Gathering (info severity)
	// Captures useful informational findings like missing security headers, SSL details, etc.
	{
		Name:        "Security-Info",
		Tags:        []string{"misconfig", "security-headers", "ssl", "tls", "cookie", "cors"},
		Severity:    []string{"info", "low"},
		Description: "Security Information Gathering (Headers, SSL, Cookies)",
		Category:    "web",
	},
}
...
func (s *Scanner) sxssScan(urls []string) []Vulnerability {
	...
	for _, line := range exec.Lines(r.Stdout) {
		...
		// Skip non-URL lines (timing info, status messages, etc.)
		if !strings.HasPrefix(line, "http://") && !strings.HasPrefix(line, "https://") {
			continue
		}

		// Skip lines that are clearly not URLs (contain timing/status text)
		if strings.Contains(line, "Total execution time") ||
			strings.Contains(line, "requests made") ||
			strings.Contains(line, "reflections found") {
			continue
		}

		// Extract the base URL (before any sxss annotation) for the URL field
		// sxss may append reflection info after the URL
		urlPart := line
		if idx := strings.Index(line, " ["); idx > 0 {
			urlPart = line[:idx]
		}

		// Extract host from URL for the Host field
		host := urlPart
...
func (s *Scanner) checkCustomPaths(scanType NucleiScanType, targetFile string) []Vulnerability {
	...
	for _, line := range exec.Lines(r.Stdout) {
		...
		// Soft-404 detection: skip responses where the title indicates "not found"
		if entry.Title != "" {
			titleLower := strings.ToLower(entry.Title)
			if strings.Contains(titleLower, "not found") ||
				strings.Contains(titleLower, "404") ||
				strings.Contains(titleLower, "page not found") {
				continue
			}
		}
