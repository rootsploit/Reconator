...
func (s *GraphQLScanner) testGraphQLEndpoint(ctx context.Context, url string) *GraphQLEndpoint {
	...
	if !strings.Contains(ct, "application/json") && !strings.Contains(ct, "application/graphql") {
		// Not a JSON response - could be SPA HTML served for all routes
		return nil
	}
	...
	// Validate the response is actual GraphQL JSON by attempting to parse it
	...
	// Must have either "data" or "errors" key in the parsed JSON (not just substring matching)
	hasData := graphqlResp.Data != nil && string(graphqlResp.Data) != "null"
	hasErrors := len(graphqlResp.Errors) > 0
	...
	if hasData && strings.Contains(bodyStr, "__schema") {
		...
		if json.Unmarshal(body, &result) == nil && len(result.Data.Schema.Types) > 0 {
			endpoint.Types = len(result.Data.Schema.Types)
		}
	} else if hasErrors {
		// Check if errors mention GraphQL-specific terms (introspection, query, mutation)
		// to confirm this is actually a GraphQL endpoint, not a generic API error
		errStr := strings.ToLower(bodyStr)
		isGraphQLError := strings.Contains(errStr, "introspection") ||
			strings.Contains(errStr, "graphql") ||
			strings.Contains(errStr, "query") ||
			strings.Contains(errStr, "must provide") ||
			strings.Contains(errStr, "syntax error") ||
			strings.Contains(errStr, "field") ||
			strings.Contains(errStr, "schema")
		if !isGraphQLError {
			return nil // Generic API error, not GraphQL
		}
...
func (s *GraphQLScanner) testGraphQLGET(ctx context.Context, url string) *GraphQLEndpoint {
	...
	// Filter soft-404s and SPA shells
	if isSoft404(bodyStr) {
		return nil
	}

	// For GET detection, require HTML content that specifically loads a GraphQL IDE
	// Matching just "graphiql" or "playground" as substrings is too loose for SPAs
	ct := resp.Header.Get("Content-Type")
	if !strings.Contains(ct, "text/html") {
		return nil
	}

	// Require GraphQL-specific script/resource loading, not just keyword mentions
	// GraphiQL loads specific JS bundles; Playground loads its own; etc.
	hasGraphiQL := strings.Contains(bodyStr, "graphiql") &&
		(strings.Contains(bodyStr, "graphiql.min.js") || strings.Contains(bodyStr, "graphiql.js") ||
			strings.Contains(bodyStr, "graphiql.min.css") || strings.Contains(bodyStr, "graphiql.css") ||
			strings.Contains(bodyStr, "react-dom") || strings.Contains(bodyStr, "graphiql-explorer"))

	hasPlayground := (strings.Contains(bodyStr, "graphql playground") || strings.Contains(bodyStr, "graphql-playground")) &&
		(strings.Contains(bodyStr, "graphql-playground") || strings.Contains(bodyStr, "middleware-"))

	hasVoyager := strings.Contains(bodyStr, "graphql-voyager") &&
		(strings.Contains(bodyStr, "voyager.worker") || strings.Contains(bodyStr, "voyager.min") ||
			strings.Contains(bodyStr, "graphql-voyager/dist"))

	hasAltair := strings.Contains(bodyStr, "altair") &&
		(strings.Contains(bodyStr, "altair-graphql") || strings.Contains(bodyStr, "altairgraphql"))

	if hasGraphiQL || hasPlayground || hasVoyager || hasAltair {
		endpoint := &GraphQLEndpoint{
			URL:  url,
			Type: detectGraphQLType(url),
		}
