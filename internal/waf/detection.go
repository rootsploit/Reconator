...
type Result struct {
	...
	WAFFingerprints map[string]string `json:"waf_fingerprints,omitempty"` // host → WAF product name
	EnrichmentOnly  bool              `json:"enrichment_only,omitempty"`  // true when run in enrichment mode
...
// Enrich performs origin IP discovery on pre-classified CDN hosts
// This is the enrichment-only mode: CDN classification already happened in the Ports phase
// via cdncheck + httpx reconciliation. WAF phase only adds origin IP discovery.
func (d *Detector) Enrich(cdnHosts, directHosts []string, cdnDetails map[string]string) (*Result, error) {
	start := time.Now()
	result := &Result{
		TotalChecked:    len(cdnHosts) + len(directHosts),
		CDNHosts:        cdnHosts,
	...
	if cdnDetails == nil {
		result.CDNDetails = make(map[string]string)
	}

	fmt.Printf("    [*] WAF Enrichment mode: %d CDN, %d direct hosts (skipping cdncheck)\n",
		len(cdnHosts), len(directHosts))

	// Attempt origin IP discovery for Cloudflare hosts
	cfHosts := d.filterCloudflareHosts(cdnHosts, result.CDNDetails)
	if len(cfHosts) > 0 {
		fmt.Printf("    [*] Attempting origin IP discovery for %d Cloudflare hosts...\n", len(cfHosts))
		origins := d.discoverOriginIPs(cfHosts)
		for host, ip := range origins {
		...
		fmt.Println("    [*] No Cloudflare hosts found, skipping origin IP discovery")
	}
