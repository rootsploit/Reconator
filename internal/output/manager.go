...
func (m *Manager) SavePortResults(result *portscan.Result) error {
	...
	// Save CDN pre-classification data (from cdncheck + httpx reconciliation)
	if len(result.CDNHosts) > 0 {
		m.saveLines(filepath.Join(dir, "cdn_hosts.txt"), result.CDNHosts)
	}
	if len(result.NonCDNHosts) > 0 {
		m.saveLines(filepath.Join(dir, "non_cdn_hosts.txt"), result.NonCDNHosts)
	}
	if len(result.CDNDetails) > 0 {
		m.saveJSON(filepath.Join(dir, "cdn_details.json"), result.CDNDetails)
	}
...
// SaveAIDecisions saves AI checkpoint decisions to the 11-ai-intel directory
func (m *Manager) SaveAIDecisions(decisions *aiguided.AIDecisions) error {
	m.results["ai_decisions"] = decisions
	dir := m.phaseDir("11-ai-intel")

	data, err := decisions.ToJSON()
	if err != nil {
		return err
	}
	return os.WriteFile(filepath.Join(dir, "ai_decisions.json"), data, 0644)
}
