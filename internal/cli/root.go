...
func init() {
	...
	// JSON output flags (for LLM agent consumption)
	rootCmd.Flags().BoolVar(&cfg.JSONOutput, "json", false, "Output unified JSON result to stdout (for LLM agents)")
	rootCmd.Flags().BoolVar(&cfg.JSONLOutput, "jsonl", false, "Stream JSONL events to stdout (for LLM agents)")
	rootCmd.Flags().BoolVar(&cfg.JSONProgress, "json-progress", false, "Output structured JSON with progress updates (for AI agents)")
	rootCmd.Flags().StringVar(&cfg.ProgressFile, "progress-file", "", "Write progress to specified JSON file")
	rootCmd.Flags().BoolVar(&cfg.WatchMode, "watch", false, "Watch output directory for incremental results")
	rootCmd.Flags().BoolVar(&cfg.Silent, "silent", false, "Silent mode: suppress banner and progress output")
	rootCmd.Flags().BoolVar(&cfg.SubsOnly, "subs-only", false, "Subdomains only mode: run subdomain phase only and output just the list")
	rootCmd.Flags().BoolVar(&cfg.MCPMode, "mcp", false, "Run as MCP server (stdin/stdout JSON)")
	...
	rootCmd.AddCommand(iprotateCmd)
}
