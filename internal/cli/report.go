...
func runReport(cmd *cobra.Command, args []string) error {
	...
	// Extract target name from directory (format: scanid_target.com or just target.com)
	dirName := filepath.Base(outDir)
	// Handle format: scanid_target.com -> target.com
	target := dirName
	if idx := strings.Index(dirName, "_"); idx > 0 {
		target = dirName[idx+1:]
	}
	...
	if data := loadJSON[osint.Result](filepath.Join(outDir, "osint", "osint_results.json")); data != nil {
		...
		if data.ASN != nil {
			parts = append(parts, fmt.Sprintf("AS%d", data.ASN.ASN))
		}
		if len(data.CertTransparency) > 0 {
			parts = append(parts, fmt.Sprintf("%d certs", len(data.CertTransparency)))
		}
		if len(data.BreachData) > 0 {
			parts = append(parts, fmt.Sprintf("%d breaches", len(data.BreachData)))
		}
		if data.DNSRecords != nil {
			parts = append(parts, "DNS")
		}
		fmt.Printf("    ✓ Loaded OSINT data: %s\n", strings.Join(parts, ", "))
		loadedCount++
	}
	...
	// Ensure BySeverity map is populated (may be nil if JSON didn't include it)
	if reportData.VulnScan != nil && reportData.VulnScan.BySeverity == nil {
		reportData.VulnScan.BySeverity = make(map[string]int)
		for _, v := range reportData.VulnScan.Vulnerabilities {
			reportData.VulnScan.BySeverity[v.Severity]++
		}
	}

	// Compute attack paths from vulnerability data
	if reportData.VulnScan != nil && len(reportData.VulnScan.Vulnerabilities) > 0 {
		...
		if len(scoredPaths) > 0 {
			fmt.Printf("    ✓ Computed %d attack paths\n", len(scoredPaths))
		}
	}
