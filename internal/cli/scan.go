...
func init() {
	...
	scanCmd.Flags().StringSliceVarP(&cfg.Phases, "phases", "p", []string{"all"}, "Phases to run (subdomain,waf,ports,takeover,historic,all)")
	...
	scanCmd.Flags().BoolVar(&cfg.StealthMode, "stealth", false, "Stealth mode: skip DNS bruteforce, permutations, scan only direct hosts")
	scanCmd.Flags().BoolVar(&cfg.FastMode, "fast", false, "Fast mode: skip alterx, limit permutations")
}
