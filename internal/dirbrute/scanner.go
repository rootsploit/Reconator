...
type Scanner struct {
	cfg              *config.Config
	c                *tools.Checker
	adaptiveWordlist string // Temp file path for AI-generated adaptive wordlist
}
...
// ScanWithAdaptiveWordlist performs directory bruteforce with AI-generated additional words
func (s *Scanner) ScanWithAdaptiveWordlist(ctx context.Context, hosts []string, additionalWords []string) (*Result, error) {
	if len(additionalWords) == 0 {
		return s.ScanWithContext(ctx, hosts)
	}

	// Create temp file with additional words
	adaptiveContent := strings.Join(additionalWords, "\n")
	adaptiveTmp, adaptiveCleanup, err := exec.TempFile(adaptiveContent, "-adaptive-wordlist.txt")
	if err != nil {
		// Fall back to normal scan
		fmt.Printf("        [DirBrute] Failed to create adaptive wordlist, using standard: %v\n", err)
		return s.ScanWithContext(ctx, hosts)
	}
	defer adaptiveCleanup()

	fmt.Printf("        [DirBrute] Created adaptive wordlist with %d AI-generated entries: %s\n",
		len(additionalWords), filepath.Base(adaptiveTmp))

	// Store adaptive wordlist path for use in scan
	s.adaptiveWordlist = adaptiveTmp
	defer func() { s.adaptiveWordlist = "" }()
...
// If an adaptive wordlist is set, it combines AI-generated words with the standard wordlist
func (s *Scanner) findWordlist() string {
	// Find the base wordlist
	var baseWordlist string
	if s.cfg.WordlistFile != "" {
		if _, err := os.Stat(s.cfg.WordlistFile); err == nil {
			baseWordlist = s.cfg.WordlistFile
		}
	}
	if baseWordlist == "" {
		baseWordlist = tools.FindDirBruteWordlist()
	}

	// If we have an adaptive wordlist, combine them
	if s.adaptiveWordlist != "" && baseWordlist != "" {
		combined, err := s.combineWordlists(baseWordlist, s.adaptiveWordlist)
		if err == nil {
			return combined
		}
		fmt.Printf("        [DirBrute] Failed to combine wordlists: %v, using base only\n", err)
	}

	// If only adaptive wordlist (no base found), use it directly
	if s.adaptiveWordlist != "" && baseWordlist == "" {
		return s.adaptiveWordlist
	}

	return baseWordlist
}

// combineWordlists merges a base wordlist with an adaptive one into a temp file
func (s *Scanner) combineWordlists(basePath, adaptivePath string) (string, error) {
	baseContent, err := os.ReadFile(basePath)
	if err != nil {
		return "", err
	}
	adaptiveContent, err := os.ReadFile(adaptivePath)
	if err != nil {
		return "", err
	}

	// Adaptive words go first (AI-prioritized paths)
	combined := string(adaptiveContent) + "\n" + string(baseContent)
	tmpPath, cleanup, err := exec.TempFile(combined, "-combined-wordlist.txt")
	if err != nil {
		return "", err
	}
	// Don't cleanup here - the caller's defer handles the scan lifecycle
	_ = cleanup
	return tmpPath, nil
}
