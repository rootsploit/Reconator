...
func GetExecutionOrder() [][]Phase {
	return [][]Phase{
		...
		// Level 2: AI Checkpoint 1 - analyzes recon surface, generates adaptive wordlist
		// Auto-skips when no AI keys configured (8s timeout with rule-based fallback)
		{PhaseAICheckpoint1},
		// Level 3: Port scanning to get alive hosts
		{PhasePorts},
		// Level 4: WAF detection needs alive hosts from ports (provides CDN filtering)
		{PhaseWAF},
		// Level 5: AI Checkpoint 2 - optimizes scan strategy based on infrastructure
		{PhaseAICheckpoint2},
		// Level 6: Depend on ports + WAF filtering (can run in parallel)
		// DirBrute now uses adaptive wordlist from AI checkpoints if available
		{PhaseTech, PhaseSecHeaders, PhaseDirBrute, PhaseVHost},
		// Level 7: Screenshot and VulnScan both depend on Tech
		// VulnScan enhanced with AI vulnerability validation
		{PhaseScreenshot, PhaseVulnScan},
		// Level 8: Final analysis with attack paths using asset model
		{PhaseAIGuided},
	}
}
...
func GetExecutionOrderForASN() [][]Phase {
	return [][]Phase{
		...
		{PhaseTakeover},
		// Level 3: AI Checkpoint 1 - analyzes recon surface
		{PhaseAICheckpoint1},
		// Level 4: Port scanning to get alive hosts
		{PhasePorts},
		// Level 5: WAF detection needs alive hosts (provides CDN filtering)
		{PhaseWAF},
		// Level 6: AI Checkpoint 2 - optimizes scan strategy
		{PhaseAICheckpoint2},
		// Level 7: Depend on ports + WAF filtering (can run in parallel)
		{PhaseTech, PhaseSecHeaders, PhaseDirBrute, PhaseVHost},
		// Level 8: Screenshot and VulnScan both depend on Tech
		{PhaseScreenshot, PhaseVulnScan},
		// Level 9: Final analysis
		{PhaseAIGuided},
	}
}
