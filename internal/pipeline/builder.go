...
func (b *Builder) Build(ctx context.Context, phase Phase) (*PhaseInput, error) {
	...
	// Skip builder output in silent mode
	isSilent := b.cfg != nil && b.cfg.Silent
	...
	for _, dep := range deps {
		if err := b.loadPhaseData(ctx, dep, input); err != nil {
			...
			if !os.IsNotExist(err) && !isStorageNotFound(err) && !isSilent {
	...
	if len(deps) > 0 && !isSilent {
		fmt.Printf("        [Builder] Loaded %d/%d dependencies for %s\n", loadedDeps, len(deps), phase)
	}

	// Load AI decisions for phases that consume them
	if phase == PhaseDirBrute || phase == PhaseVulnScan || phase == PhaseAIGuided {
		b.loadAIDecisions(ctx, input)
	}
...
func (b *Builder) loadPhaseData(ctx context.Context, phase Phase, input *PhaseInput) error {
	...
	case PhaseAIGuided:
		...
		return nil
	case PhaseAICheckpoint1:
		// Checkpoint 1 needs subdomain + historic + JSAnalysis data (already loaded via deps)
		return nil
	case PhaseAICheckpoint2:
		// Checkpoint 2 needs ports + WAF + tech data (already loaded via deps)
		return nil
	}
...
func (b *Builder) getPhaseOutputPath(phase Phase) string {
	...
	dirMap := map[Phase]string{
		...
		PhaseAICheckpoint1: "11-ai-intel",
		PhaseAICheckpoint2: "11-ai-intel",
	}
	...
	fileMap := map[Phase]string{
		...
		PhaseAICheckpoint1: "ai_decisions.json",
		PhaseAICheckpoint2: "ai_decisions.json",
	}
...
func (b *Builder) parsePortsOutput(data []byte, input *PhaseInput) error {
	...
	var output struct {
		...
		CDNFallback bool              `json:"cdn_fallback"`
	}
	if err := json.Unmarshal(data, &output); err != nil {
		if !isSilent {
	...
	// CDN pre-classification data from Ports phase
	...
	if len(output.CDNDetails) > 0 {
		input.CDNDetails = output.CDNDetails
	}
	// CRITICAL FALLBACK: If no alive hosts and no CDN/direct classification,
	// but we have hosts with open ports, use those as direct hosts
	if len(input.AliveHosts) == 0 && len(input.DirectHosts) == 0 && len(input.CDNHosts) == 0 && len(input.OpenPorts) > 0 {
		if !isSilent {
			fmt.Printf("        [Builder] No alive hosts but %d hosts with open ports - using as direct hosts\n", len(input.OpenPorts))
		}
		input.DirectHosts = make([]string, 0, len(input.OpenPorts))
		for host := range input.OpenPorts {
			input.DirectHosts = append(input.DirectHosts, host)
		}
	}
	if !isSilent {
		fmt.Printf("        [Builder] Parsed %d alive_hosts, %d non-CDN, %d CDN",
			len(input.AliveHosts), len(input.DirectHosts), len(input.CDNHosts))
		if output.CDNFallback {
			fmt.Printf(" (cdncheck fallback)")
		}
		fmt.Println()
	}
	return nil
...
// loadAIDecisions loads AI checkpoint decisions from the 11-ai-intel directory
func (b *Builder) loadAIDecisions(ctx context.Context, input *PhaseInput) {
	isSilent := b.cfg != nil && b.cfg.Silent
	path := filepath.Join("11-ai-intel", "ai_decisions.json")
	data, err := b.storage.Read(ctx, path)
	if err != nil {
		// AI decisions are optional - not an error if missing
		return
	}

	decisions, err := aiguided.FromJSON(data)
	if err != nil {
		if !isSilent {
			fmt.Printf("        [Builder] Warning: Failed to parse AI decisions: %v\n", err)
		}
		return
	}

	input.AIDecisions = decisions
	if !isSilent {
		fmt.Printf("        [Builder] Loaded AI decisions: %d prioritized hosts, %d wordlist entries, %d nuclei tags\n",
			len(decisions.PrioritizedHosts), len(decisions.GetAllWordlistEntries()), len(decisions.NucleiTags))
	}
}
