...
type PhaseInput struct {
	...
	// From waf/ports phase (CDN pre-classification)
	DirectHosts []string          // Non-WAF/CDN protected hosts
	CDNHosts    []string          // WAF/CDN protected hosts
	CDNDetails  map[string]string // host → CDN provider name (from ports pre-classification)
	...
	// From AI checkpoints (nil when no AI keys configured)
	AIDecisions *aiguided.AIDecisions
}
...
// HasTargets returns true if there are any targets to scan (alive hosts, CDN hosts, or direct hosts)
func (p *PhaseInput) HasTargets() bool {
	return len(p.AliveHosts) > 0 || len(p.CDNHosts) > 0 || len(p.DirectHosts) > 0
}
...
// Prefers HttpxURLs (full URLs with protocol), falls back to hostnames, then CDN hosts
func (p *PhaseInput) GetScreenshotTargets() []string {
	...
	// Next fallback: AliveHosts from port scan
	if len(p.AliveHosts) > 0 {
		return p.AliveHosts
	}
	// Last resort: CDN hosts (they responded to HTTP, just not in port scan)
	if len(p.CDNHosts) > 0 {
		urls := make([]string, 0, len(p.CDNHosts))
		for _, host := range p.CDNHosts {
			urls = append(urls, "https://"+host)
		}
		return urls
	}
