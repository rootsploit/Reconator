...
func RegisterAllAdapters(exec *Executor, cfg *config.Config, checker *tools.Checker) {
	...
	// AI Checkpoints - auto-skip when no AI keys configured
	exec.Register(NewAICheckpoint1Adapter(cfg))
	exec.Register(NewAICheckpoint2Adapter(cfg))
}
...
func (a *WAFAdapter) Execute(ctx context.Context, input *PhaseInput) (*PhaseResult, error) {
	...
	// Check if CDN pre-classification happened in Ports phase
	// If yes → enrichment mode (skip cdncheck, only do origin IP discovery)
	// If no → fall back to full detection mode (backward compat)
	if len(input.CDNHosts) > 0 || len(input.DirectHosts) > 0 {
		// Enrichment mode: Ports phase already did cdncheck + httpx reconciliation
		fmt.Printf("        [WAFAdapter] Enrichment mode: %d CDN, %d direct (from Ports pre-classification)\n",
			len(input.CDNHosts), len(input.DirectHosts))

		res, err := a.detector.Enrich(input.CDNHosts, input.DirectHosts, input.CDNDetails)
		result.EndTime = time.Now()
		result.Duration = result.EndTime.Sub(start)
	...
	// Fallback: No pre-classification data — run full CDN detection
	...
	fmt.Printf("        [WAFAdapter] Full detection mode with %d alive hosts\n", len(input.AliveHosts))
...
func (a *TechAdapter) Execute(ctx context.Context, input *PhaseInput) (*PhaseResult, error) {
	...
	// Check for targets - if no alive hosts, use CDN hosts as fallback
	// Then fallback to direct hosts, then to hosts with open ports (even if httpx didn't confirm HTTP)
	targets := input.AliveHosts
	if len(targets) == 0 && len(input.CDNHosts) > 0 {
		fmt.Printf("        [TechAdapter] No alive hosts from port scan, using %d CDN hosts\n", len(input.CDNHosts))
		targets = input.CDNHosts
	}
	// Fallback to direct hosts from WAF detection
	if len(targets) == 0 && len(input.DirectHosts) > 0 {
		fmt.Printf("        [TechAdapter] No alive hosts, using %d direct hosts (non-WAF)\n", len(input.DirectHosts))
		targets = input.DirectHosts
	}
	// Fallback to hosts with detected open ports (naabu found ports but httpx may have failed)
	if len(targets) == 0 && len(input.OpenPorts) > 0 {
		fmt.Printf("        [TechAdapter] No alive hosts, using %d hosts with open ports (httpx may have failed)\n", len(input.OpenPorts))
		targets = make([]string, 0, len(input.OpenPorts))
		for host := range input.OpenPorts {
			targets = append(targets, host)
		}
	}

	if len(targets) == 0 {
		fmt.Printf("        [TechAdapter] Skipping: no targets (AliveHosts=%d, CDNHosts=%d, OpenPorts=%d)\n", len(input.AliveHosts), len(input.CDNHosts), len(input.OpenPorts))
		result.Status = StatusSkipped
		result.EndTime = time.Now()
		result.Duration = result.EndTime.Sub(start)
		return result, nil
	}
	fmt.Printf("        [TechAdapter] Starting detection with %d targets\n", len(targets))

	res, err := a.detector.Detect(targets)
	result.EndTime = time.Now()
	result.Duration = result.EndTime.Sub(start)
...
func (a *SecHeadersAdapter) Execute(ctx context.Context, input *PhaseInput) (*PhaseResult, error) {
	...
	targets := input.AliveHosts
	if len(targets) == 0 && len(input.CDNHosts) > 0 {
		fmt.Printf("        [SecHeadersAdapter] No alive hosts from port scan, using %d CDN hosts\n", len(input.CDNHosts))
		targets = input.CDNHosts
	}
	// Fallback to direct hosts from WAF detection
	if len(targets) == 0 && len(input.DirectHosts) > 0 {
		fmt.Printf("        [SecHeadersAdapter] No alive hosts, using %d direct hosts (non-WAF)\n", len(input.DirectHosts))
		targets = input.DirectHosts
	}
	// Fallback to hosts with detected open ports (naabu found ports but httpx may have failed)
	if len(targets) == 0 && len(input.OpenPorts) > 0 {
		fmt.Printf("        [SecHeadersAdapter] No alive hosts, using %d hosts with open ports (httpx may have failed)\n", len(input.OpenPorts))
		targets = make([]string, 0, len(input.OpenPorts))
		for host := range input.OpenPorts {
			targets = append(targets, host)
		}
	...
	if len(targets) == 0 {
		fmt.Printf("        [SecHeadersAdapter] Skipping: no targets (AliveHosts=%d, CDNHosts=%d, OpenPorts=%d)\n", len(input.AliveHosts), len(input.CDNHosts), len(input.OpenPorts))
		result.Status = StatusSkipped
		result.EndTime = time.Now()
		result.Duration = result.EndTime.Sub(start)
		return result, nil
	}
	fmt.Printf("        [SecHeadersAdapter] Starting security header check with %d targets\n", len(targets))

	res, err := a.checker.Check(input.Target, targets)
	result.EndTime = time.Now()
	result.Duration = result.EndTime.Sub(start)
...
func (a *VulnScanAdapter) Execute(ctx context.Context, input *PhaseInput) (*PhaseResult, error) {
	...
	// Check for targets - if no alive hosts, use CDN hosts as fallback (they're accessible via HTTP)
	// Then fallback to direct hosts, then to hosts with open ports (even if httpx didn't confirm HTTP)
	targets := input.AliveHosts
	if len(targets) == 0 && len(input.CDNHosts) > 0 {
		fmt.Printf("        [VulnScanAdapter] No alive hosts from port scan, using %d CDN hosts\n", len(input.CDNHosts))
		targets = make([]string, len(input.CDNHosts))
		for i, host := range input.CDNHosts {
			targets[i] = "http://" + host
		}
	}
	// Fallback to direct hosts from WAF detection
	if len(targets) == 0 && len(input.DirectHosts) > 0 {
		fmt.Printf("        [VulnScanAdapter] No alive hosts, using %d direct hosts (non-WAF)\n", len(input.DirectHosts))
		targets = make([]string, len(input.DirectHosts))
		for i, host := range input.DirectHosts {
			targets[i] = "http://" + host
		}
	}
	// Fallback to hosts with detected open ports (naabu found ports but httpx may have failed)
	if len(targets) == 0 && len(input.OpenPorts) > 0 {
		fmt.Printf("        [VulnScanAdapter] No alive hosts, using %d hosts with open ports (httpx may have failed)\n", len(input.OpenPorts))
		targets = make([]string, 0, len(input.OpenPorts))
		for host := range input.OpenPorts {
	...
	if len(targets) == 0 {
		fmt.Printf("        [VulnScanAdapter] Skipping: no targets (AliveHosts=%d, CDNHosts=%d, OpenPorts=%d)\n", len(input.AliveHosts), len(input.CDNHosts), len(input.OpenPorts))
		result.Status = StatusSkipped
		result.EndTime = time.Now()
		result.Duration = result.EndTime.Sub(start)
		return result, nil
	}
	fmt.Printf("        [VulnScanAdapter] Starting scan with %d targets\n", len(targets))

	// CRITICAL FIX: Run katana active crawl now that targets are available
	...
	// Run katana to actively crawl targets and discover fresh endpoints
	if !a.cfg.PassiveMode && a.checker.IsInstalled("katana") && len(targets) > 0 {
		fmt.Println("        [VulnScan] Running katana active crawl on targets...")
		mergedURLs, mergedCategorized, _ := a.collector.RunKatanaAndMerge(targets, allURLs, input.Target)
		allURLs = mergedURLs
	...
	if err == nil && graphqlResult != nil && graphqlResult.TotalFound > 0 {
		...
		// Save GraphQL results to 8-vulnscan folder (not separate graphql/ folder)
		graphqlDir := filepath.Join(input.Config.OutputDir, input.Target, "8-vulnscan")
		if err := graphqlResult.SaveGraphQLResults(graphqlDir); err != nil {
			fmt.Printf("        [VulnScan] Warning: failed to save GraphQL results: %v\n", err)
		}
	}
	...
	if err == nil && cloudResult != nil && len(cloudResult.Buckets) > 0 {
		...
		for _, bucket := range cloudResult.Buckets {
			severity := "info"
			vulnName := fmt.Sprintf("%s Bucket Discovered", strings.ToUpper(bucket.Provider))
			...
			case "private":
				severity = "info"
				vulnName = fmt.Sprintf("%s Bucket Private (Not Accessible)", strings.ToUpper(bucket.Provider))
				description += " - access denied (bucket exists but is private)"
			}
...
func (a *DirBruteAdapter) Execute(ctx context.Context, input *PhaseInput) (*PhaseResult, error) {
	...
	// Check if AI decisions provide adaptive wordlist entries
	var additionalWords []string
	if input.AIDecisions != nil && input.AIDecisions.HasAdaptiveWordlist() {
		additionalWords = input.AIDecisions.GetAllWordlistEntries()
		fmt.Printf("        [DirBruteAdapter] Using %d AI-generated wordlist entries\n", len(additionalWords))
	}

	// Use context-aware scanning with optional adaptive wordlist
	var res *dirbrute.Result
	var err error
	if len(additionalWords) > 0 {
		res, err = a.scanner.ScanWithAdaptiveWordlist(phaseCtx, hosts, additionalWords)
	} else {
