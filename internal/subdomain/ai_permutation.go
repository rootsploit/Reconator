...
// buildEnhancedPrompt uses TOON format for compact encoding
func (p *AIPermutator) buildEnhancedPrompt(domain string, prefixes []string, tokenAnalysis *TokenAnalysis) string {
	...
	topPrefixes := getTopN(tokenAnalysis.Prefixes, 8)
	observedSamples := prefixes
	if len(observedSamples) > 20 {
		observedSamples = observedSamples[:20]
	}

	// Build TOON data
	data := map[string]interface{}{
		"domain":     domain,
		"delimiter":  tokenAnalysis.Delimiter,
		"count":      len(prefixes),
		"tokens":     topTokens,
		"prefixes":   topPrefixes,
		"subdomains": observedSamples,
	}

	// Simple prompt using TOON
	return `Analyze these subdomains and predict 30 new ones.

` + aiguided.ToonObject(data) + `

Include: env variants (dev, staging, test), infra (vpn, gitlab, jenkins), cloud (eks, k8s, s3).
Respond JSON: {"predictions":["..."],"patterns":["..."]}`
}
