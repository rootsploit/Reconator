...
func (s *SSLCertRecon) isSilent() bool {
...
// fetchIPRanges fetches IP ranges for a cloud provider using unified freshness cache
func (s *SSLCertRecon) fetchIPRanges(provider string) ([]string, error) {
	// Use unified freshness-based cache instead of per-file 24hr TTL
	cacheDir := filepath.Join(os.TempDir(), "reconator", "ipranges")
	allRanges, err := iprange.FetchWithFreshness(cacheDir, false)
	if err != nil {
		return nil, fmt.Errorf("failed to fetch IP ranges: %w", err)
	}

	ranges, ok := allRanges[provider]
	if !ok {
