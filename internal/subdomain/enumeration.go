...
func (e *Enumerator) isSilent() bool {
	return e.cfg != nil && e.cfg.Silent
}
...
func (e *Enumerator) Enumerate(domain string) (*Result, error) {
	...
	// Check for wildcard DNS before enumeration (skip if silent mode)
	if !e.cfg.Silent {
		fmt.Println("    [*] Checking for wildcard DNS...")
	}
	hasWildcard, wildcardIPs := e.checkWildcard(domain)
	if hasWildcard && !e.cfg.Silent {
	...
	// In quick mode with subs-only, skip slow enumeration tools for maximum speed
	// Only add crt.sh which is extremely fast
	if e.cfg.QuickMode && e.cfg.SkipDNSBrute {
		// Ultra-fast mode: only crt.sh (if available) and chaos
		...
		// crt.sh is extremely fast (~1s), always include it
		tools = append(tools, struct {
			name string
			fn   func(string) []string
		}{"crt.sh", e.crtSh})
	...
	// Only add additional tools in normal mode, not quick mode
	if !(e.cfg.QuickMode && e.cfg.SkipDNSBrute) {
	...
	// Only add if not already added in quick mode
	if !(e.cfg.QuickMode && e.cfg.SkipDNSBrute) {
		tools = append(tools, struct {
			name string
			fn   func(string) []string
	...
	} else if len(current) >= maxSubs && !e.isSilent() {
...
func (e *Enumerator) subfinder(domain string) []string {
	...
	// Use shorter timeout in quick mode for faster results
	timeout := 5 * time.Minute
	if e.cfg.QuickMode {
		timeout = 10 * time.Second
	}
	r := exec.Run("subfinder", args, &exec.Options{Timeout: timeout})
	if r.Error != nil {
		return nil
	}
...
// crt.sh is an extremely fast certificate transparency log search
// Returns subdomains in <1 second typically
func (e *Enumerator) crtSh(domain string) []string {
	url := fmt.Sprintf("https://crt.sh/?q=%s&output=json", domain)

	// Use curl since it's universally available
	args := []string{"-s", "-L", url}
	r := exec.Run("curl", args, &exec.Options{Timeout: 10 * time.Second})
	...
	// Extract subdomains from both common_name and name_value fields
	seen := make(map[string]bool)
	var subs []string
	...
	for _, rec := range records {
		...
		if rec.CommonName != "" && rec.CommonName != "*" {
			if strings.HasSuffix(rec.CommonName, "."+domain) || rec.CommonName == domain {
				if !seen[rec.CommonName] {
					seen[rec.CommonName] = true
					subs = append(subs, rec.CommonName)
				}
			}
		}
	}
...
func (e *Enumerator) assetfinder(domain string) []string {
	...
	r := exec.Run("assetfinder", []string{"--subs-only", domain}, &exec.Options{Timeout: timeout})
	if r.Error != nil {
		return nil
	}
	return exec.Lines(r.Stdout)
