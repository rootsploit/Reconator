...
func (a *Analyzer) Analyze(ctx context.Context, jsURLs []string) (*Result, error) {
	...
	fmt.Printf("        Found: %d endpoints, %d DOM XSS sinks, %d sources, %d taint flows, %d secrets, %d API paths\n",
		len(result.Endpoints), len(result.DOMXSSSinks), len(result.DOMXSSSources),
		len(result.TaintFlows), len(result.Secrets), len(result.APIPaths))
