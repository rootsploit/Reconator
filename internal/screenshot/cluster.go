...
type ScreenshotMeta struct {
	...
	// ImageData stores the actual screenshot as base64 for portability
	// This allows reports to be self-contained without needing separate image files
...
func processScreenshot(filePath string) (*ScreenshotMeta, error) {
	...
	// Read the image file and embed as base64 for portable reports
	imageData := ""
	if imgData, err := os.ReadFile(filePath); err == nil {
		// Only embed if image is reasonable size (skip very large images)
		if len(imgData) < 2*1024*1024 { // 2MB limit
