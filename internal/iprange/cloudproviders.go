...
func FetchCloudProviderRanges(provider string) ([]string, error) {
	url, ok := cloudProviderSources[strings.ToLower(provider)]
	if !ok {
		// Check CDN sources
		url, ok = cdnProviderSources[strings.ToLower(provider)]
		if !ok {
...
func (s *CloudReconScanner) ScanAllProviders(targetDomain string, samplePerProvider int) ([]*CloudReconResult, error) {
	...
	providers := []string{"aws", "azure", "gcp", "oracle", "digitalocean"}
