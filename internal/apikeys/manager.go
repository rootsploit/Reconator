...
type Config struct {
	...
	// Fleet configuration for distributed scanning
	Fleet FleetKeys `yaml:"fleet,omitempty"`

	// IP rotation configuration
	IPRotate IPRotateKeys `yaml:"iprotate,omitempty"`
}

// FleetKeys holds distributed scanning fleet configuration
type FleetKeys struct {
	Backend    string   `yaml:"backend,omitempty"`      // "local", "ssh" (default: "local")
	Workers    int      `yaml:"workers,omitempty"`      // Number of workers (default: 1)
	SSHUser    string   `yaml:"ssh_user,omitempty"`     // SSH username (default: "root")
	SSHKeyFile string   `yaml:"ssh_key_file,omitempty"` // SSH private key path
	SSHPort    int      `yaml:"ssh_port,omitempty"`     // SSH port (default: 22)
	SSHHosts   []string `yaml:"ssh_hosts,omitempty"`    // Pre-existing SSH hosts
	SetupCmd   string   `yaml:"setup_cmd,omitempty"`    // Post-provision setup command

	// Cloud provisioning (opt-in, has cost implications)
	Provider    string `yaml:"provider,omitempty"`     // "digitalocean", "aws"
	APIKey      string `yaml:"api_key,omitempty"`      // Cloud API key or env var
	Region      string `yaml:"region,omitempty"`       // Cloud region
	Size        string `yaml:"size,omitempty"`         // Instance size
	SSHKeyID    string `yaml:"ssh_key_id,omitempty"`   // Pre-uploaded SSH key ID in cloud
	SpotEnabled bool   `yaml:"spot_enabled,omitempty"` // Use spot/preemptible (AWS)
}

// IPRotateKeys holds IP rotation configuration
type IPRotateKeys struct {
	Enabled   bool     `yaml:"enabled,omitempty"`    // Must be explicitly enabled
	ProxyPort int      `yaml:"proxy_port,omitempty"` // Local proxy port (default: 8888)
	Regions   []string `yaml:"regions,omitempty"`    // AWS regions for API Gateways
}
...
type AIKeys struct {
	...
	// DeepSeek
	DeepSeek []string `yaml:"deepseek,omitempty" json:"deepseek,omitempty"`
	// Kimi (Moonshot AI)
	Kimi []string `yaml:"kimi,omitempty" json:"kimi,omitempty"`
	// Minimax AI
	Minimax []string `yaml:"minimax,omitempty" json:"minimax,omitempty"`
	// Ollama (local)
...
func (m *Manager) loadFromEnv() {
	...
	aiEnvMappings := map[string]*[]string{
		...
		"KIMI_API_KEY":      &m.config.AI.Kimi,
		"MOONSHOT_API_KEY":  &m.config.AI.Kimi,
		"MINIMAX_API_KEY":   &m.config.AI.Minimax,
	}
...
func (m *Manager) importFromAIConfig() int {
	...
	for _, provider := range aiConfig.Providers {
		...
		case "kimi", "moonshot":
			target = &m.config.AI.Kimi
		case "minimax":
			target = &m.config.AI.Minimax
		case "ollama":
...
func (m *Manager) HasAIKeys() bool {
	...
	return len(a.OpenAI) > 0 || len(a.Claude) > 0 || len(a.Gemini) > 0 ||
		len(a.Groq) > 0 || len(a.DeepSeek) > 0 || len(a.Kimi) > 0 ||
		len(a.Minimax) > 0 || a.Ollama.URL != ""
}
...
func (m *Manager) GetKeyCount() int {
	...
	// AI keys
	a := m.config.AI
	count += len(a.OpenAI) + len(a.Claude) + len(a.Gemini) + len(a.Groq) + len(a.DeepSeek) + len(a.Kimi) + len(a.Minimax)
	if a.Ollama.URL != "" {
		count++
	}
...
func (m *Manager) TestOSINTKey(provider string, key string) TestResult {
	...
	case "dnsdb":
		testURL = fmt.Sprintf("https://api.dnsdb.info/lookup/rdns/example.com?apikey=%s", key)
	case "securityscorecard":
		testURL = "https://api.securityscorecard.io/v1/metrics"
		headers = map[string]string{"Authorization": "Token " + key}
	default:
		// Generic fallback: assume provider has an API endpoint at api.<provider>.io or similar
		// This is a best-effort test - user should verify manually
		testURL = "https://api." + strings.ToLower(provider) + ".io/v1/status"
		headers = map[string]string{"Authorization": "Bearer " + key}
	}
...
func (m *Manager) TestAIKey(provider string, key string) TestResult {
	...
	case "minimax":
		...
		headers = map[string]string{"Authorization": "Bearer " + key}
	case "togetherai":
		testURL = "https://api.together.ai/v1/models"
		headers = map[string]string{"Authorization": "Bearer " + key}
	case "anyscale":
		testURL = "https://api.anyscale.com/v1/models"
		headers = map[string]string{"Authorization": "Bearer " + key}
	case "perplexity":
	...
	case "cohere":
		...
		headers = map[string]string{"Authorization": "Bearer " + key}
	case "azure":
		// Azure OpenAI uses endpoint-based auth - test with OpenAI-compatible endpoint
		testURL = "https://api.openai.com/v1/models"
		headers = map[string]string{"Authorization": "Bearer " + key}
	default:
		// Generic fallback for unknown LLM providers
		testURL = "https://api." + strings.ToLower(provider) + ".com/v1/models"
		headers = map[string]string{"Authorization": "Bearer " + key}
	}
...
func (m *Manager) TestAllKeys() []TestResult {
	...
	aiKeys := map[string][]string{
		...
		"kimi":     m.config.AI.Kimi,
		"minimax":  m.config.AI.Minimax,
	}
...
ai:
  ...
  # Minimax - Minimax AI (https://minimaxi.com)
  minimax: []
...
# ============================================================================
# Distributed Scanning (OPTIONAL)
# ============================================================================
# TIP: For simple distributed scanning, use: reconator scan target --distributed --provider=aws
# The fleet config below is for advanced persistent fleet setups.
# fleet:
#   backend: ssh              # "local" (default) or "ssh"
#   workers: 3                # Number of distributed workers
#   ssh_user: root
#   ssh_key_file: ~/.ssh/id_rsa
#   ssh_port: 22
#   ssh_hosts:                # Pre-existing hosts (no cloud provisioning needed)
#     - 10.0.0.1
#     - 10.0.0.2
#   # Cloud provisioning (creates billable resources)
#   # provider: aws           # "digitalocean" or "aws"
#   # api_key: $DO_TOKEN      # Or use environment variable
#   # region: us-east-1
#   # size: t3.micro          # DO: s-1vcpu-1gb, AWS: t3.micro
#   # spot_enabled: true      # AWS spot instances

# ============================================================================
# IP Rotation via AWS API Gateway (OPTIONAL)
# ============================================================================
# Requires: AWS CLI configured with valid credentials.
# iprotate:
#   enabled: false            # Must be explicitly enabled
#   proxy_port: 8888          # Local proxy port
#   regions:                  # AWS regions for API Gateway endpoints
#     - us-east-1
#     - us-west-2
#     - eu-west-1
`
